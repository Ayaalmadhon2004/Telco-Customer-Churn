...
# 2️⃣ قراءة البيانات (ضع الملف في نفس المجلد أو عدلي المسار)
...
# 9️⃣ التحليل حسب مدة الاشتراك (tenure) وعلاقة Churn
plt.figure(figsize=(8,5))
sns.boxplot(x="Churn", y="tenure", data=data)
plt.title("مدة الاشتراك vs ترك العميل")
plt.show()

# 10️⃣ العلاقة بين المبلغ الكلي والفئة Churn
plt.figure(figsize=(8,5))
sns.boxplot(x="Churn", y="TotalCharges", data=data)
plt.title("إجمالي الفاتورة vs ترك العميل")
plt.show()

# 11️⃣ إحصائيات عامة للبيانات الرقمية
print(data.describe())

# 12️⃣ فحص توازن الفئات
print(data["Churn"].value_counts())
print(data["Churn"].value_counts(normalize=True))  # بالنسبة المئوية

# -------------------------
# الكود جاهز الآن لتحليل إضافي أو بناء نموذج تنبؤي (ML)
